...
// FUNCTION
const { checkNameUrl, getDBEps, addNewEps, addNewanime, checkNameUrls, checkThumb, objGenre, searchAnime, addView, AllGenre, addReport, removeReport, checkData, loopGenreALL, translateAnime } = require("../function/function")
  ...
  start() {
    ...
    app.post('/admin', async (req, res) => {
      ...
      if (act === 'add') {
        ...
        if (parseInt(acnum) < 1 || parseInt(acnum) > 3 || parseInt(acnum) === undefined || isNaN(parseInt(acnum))) {
          ...
          if (!checkNameUrls(nameurl)) return res.send(this.notFound("Wrong Anime", "404 Not Found"));
          if (typeanime != "MOVIE" && typeanime != "TV") return res.send(this.notFound("Wrong Type", "404 Not Found"));
          data.addNewEps(nameurl, anime, typeanime.toUpperCase(), downloadurl, '', stream, parseInt(eps), date)
          return res.redirect('/')
        } else {
          return res.redirect('/')
        }
        ...
        var { nameurl, download, stream } = req.body
        if (parseInt(acnum) < 1 || parseInt(acnum) > 3 || parseInt(acnum) === undefined || isNaN(parseInt(acnum))) {
          return res.send(this.notFound("Wrong Action", "404 Not Found"));
        } else {
    ...
    app.get('/anime/:anime', async (req, res) => {
      ...
      translateAnime(checkName.nama).then((data) => {
        res.send(this.detailPage(checkName, this.loopingEps(checkName), this.loopingGenre(checkName), filterView, data))
      })
    })
...
<div class="series-bg" style="background-image:url(`+ checkName.thumb + `);"></div>
</div>
<div class="container">
<div class="series-flex">
<div class="series-flexleft">
<div class="series-thumb">
<img src="${checkName.thumb}?resize=225,310" alt="${checkName.nama}" title="${checkName.nama}"></div>
...
<li><b>Studio</b><span>${checkName.studio}</span></li>
<li><b>View</b><span>${checkName.view}</span></li>
</ul>
</div>
</div>
...
<a href="/anime/${data.nameurl}/${data.eps[i].eps}" title="${data.nama} Episode ${data.eps[i].eps} Sub Indo"><span>Episode ${data.eps[i].eps}${i + 1 === data.eps.length ? data.status === true ? " - End" : "" : ""}</span><span class="date">${data.time}</span></a>
</div>
</div>
</li>`
    }
  ...
  WatchAnime = (urlvid, data, eps, loopeps, loopdow, genre, dataeps) => {
...
<a class="btnx dbox" href="${data.eps[parseInt(eps) - 1].download[0].url}"><i class="fa-solid fa-file-arrow-down"></i> Download</a></div> 
</div> 
</div>
</div>
...
<span class="infoz">${moment(data.eps[parseInt(eps) - 1].time).tz('Asia/Jakarta').format('MMMMM DD, YYYY')} - ${data.eps[parseInt(eps) - 1].view} Views</span>
</div>
<div class="series-thumb">
<a href="/series/seirei-gensouki/" title="${data.name} Episode ${eps} Sub Indo">
<img src="${data.thumb}?resize=225,310" alt="${data.name}" title="${data.name}"></a>
</div>
...
${allpage - 1 === parseInt(page) ? `` : allpage  === 1 ? `` : `<span aria-current="page" class="page-numbers current">${parseInt(page)}</span>`}
...
${allpage - 1 === parseInt(page) ? allpage === 1 ? `` : `<span aria-current="page" class="page-numbers current">${parseInt(page)}</span>` : ``}
${parseInt(page) + 5  > allpage - 1 ? `` : `<a class="page-numbers">..</a>`} 
${parseInt(page) + 5  > allpage - 1 ? `` : `<a class="page-numbers" href="/page/${allpage - 1}">${allpage - 1}</a>`} 
...
<img src="${thumbC}" alt="${data[i].nama}" title="${data[i].nama}"><div class="flexbox-type ${dataC.type}">${dataC.type}</div>
<div class="flexbox-status ${dataC.status === true ? "Completed" : "Ongoing"}">${dataC.status === true ? "Completed" : "Ongoing"}</div>
<div class="flexbox-episode"><span class="eps">Episode</span><span>${data[i].eps}${dataC.status === true ? " - End" : ""}</span></div>
</div>
<div class="flexbox-title">Episode ${data[i].eps}</div>
</a>
</div>`
        }
      }
...
<a href="${data[i].url}/${data[i].eps}" title="${data[i].nama}">
<div class="flexbox-thumb"> 
<img src="${thumbC}" alt="${data[i].nama}" title="${data[i].nama}"><div class="flexbox-type ${data[i].type}">${data[i].type}</div>
<div class="flexbox-status ${dataC.status === true ? "Completed" : "Ongoing"}">${dataC.status === true ? "Completed" : "Ongoing"}</div>
<div class="flexbox-episode"><span class="eps">Episode</span><span>${data[i].eps}${dataC.status === true ? " - End" : ""}</span></div>
</div>
<div class="flexbox-title">Episode ${data[i].eps}</div>
  ...
  topviewAnime = (data) => {
    ...
    if (data.length < 7) {
      ...
      for (let i = 0; i < data.length; i++) {
        textnya += `<div class='flexbox-item'><a href='/anime/${data[i].nameurl}' title='${data[i].nameurl}'><div class='flexbox-thumb'><img src="${data[i].thumb}" alt="${data[i].nama}" title="${data[i].nama}"><div class='flexbox-number'>${i + 1}</div></div><div class='flexbox-title'>${data[i].nama}</div></a></div>`
      }
      return textnya
    } else {
      data.sort((a, b) => (a.view < b.view) ? 1 : -1)
...
<div class="flexeps-infoz">
<a href="/${data.nameurl}/${data.eps[i].eps}" title="${data.nama} Episode ${data.eps[i].eps} Sub Indo"><span>Episode ${data.eps[i].eps} ${i + 1 === data.eps.length ? data.status === true ? ' - End' : '' : ''}</span><span class="date">${moment(data.eps[i].time).tz('Asia/Jakarta').format('MMMMM DD, YYYY')}</span></a>
</div>
</div>
</li>`
    }
  ...
  genreList = (data) => {
    ...
    for (let i = 0; i < AllGenre(this.allanime).length; i++) {
      textnya += `<li><a href="/genre/${AllGenre(this.allanime)[i]}" title="Lihat Anime ${AllGenre(this.allanime)[i]}">${AllGenre(this.allanime)[i]}<span>${this.genreCheck(AllGenre(this.allanime)[i])}</span></a></li>`
    }
    return textnya
  }
  genreMove = (data, page) => {
...
<img src="${data[i].thumb}?resize=225,310" alt="${data[i].nama}" title="${data[i].nama}"><div class="flexbox2-title"><span>${data[i].nama}</span><span class="studio">${data[i].studio}</span></div>
</div>
</a>
...
<div class="genres"><span>${genrenya}</div>
      ...
      const filterPage = pageX !== 1 ? 0 + 12 * pageFil : 0
      const filterLow = pageX !== 1 ? 12 * pageX : 12
  ...
  GetHtmlList = (data) => {
    ...
    if (data.h.length !== 0) {
      ...
      for (let i = 0; i < data.h.length; i++) {
        textnya += `<li class="${data.$[i].type}"><a class="series" rel="6" href="/anime/${data.h[i].nameurl}">${data.h[i].nama}</a></li>`
      }
    }
    if (data.i.length !== 0) {
