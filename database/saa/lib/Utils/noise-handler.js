...
const makeNoiseHandler = ({ public: publicKey, private: privateKey }) => {
    const authenticate = (data) => {
        if (!isFinished) {
            hash = (0, crypto_1.sha256)(Buffer.from(WABinary_1.Binary.build(hash, data).readByteArray()));
        }
    };
    const encrypt = (plaintext) => {
        const authTagLength = 128 >> 3;
        const cipher = (0, crypto_2.createCipheriv)('aes-256-gcm', encKey, generateIV(writeCounter), { authTagLength });
        cipher.setAAD(hash);
    ...
    const decrypt = (ciphertext) => {
        ...
        const cipher = (0, crypto_2.createDecipheriv)('aes-256-gcm', decKey, iv);
